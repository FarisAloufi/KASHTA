...
import { Trash2, Plus, Minus, ShoppingBag, MapPin, Calendar, ArrowRight, Loader, CheckCircle, Package, ShieldCheck, Clock } from "lucide-react";
            ...
            {cartItems.length === 0 ? (
              ...
              <div className="p-6 md:p-10">
                ...
                <div className="space-y-6 mb-12">
                  {cartItems.map(item => (
                    <div key={item.cartId} className="flex flex-col md:flex-row items-center gap-6 p-4 rounded-2xl border border-main-text/5 bg-main-bg/5 hover:border-main-accent/30 transition-colors">
                      ...
                      <div className="flex-1 text-center md:text-right w-full">
                        ...
                        <p className="text-main-text/50 text-xs font-medium">سعر الوحدة: {Number(item.servicePrice).toLocaleString("ar-SA")} ريال</p>
                      </div>

                      <div className="flex items-center bg-white/50 rounded-xl p-1 border border-main-text/5 shadow-inner">
                        <button onClick={() => updateCartItemQuantity(item.cartId, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center bg-white text-main-text rounded-lg hover:bg-main-accent hover:text-second-text transition shadow-sm">
                      ...
                      <div className="text-center md:text-left min-w-[100px]">
                        ...
                        <p className="font-black text-xl text-main-text">
                          {(item.servicePrice * item.quantity).toLocaleString("ar-SA")} <span className="text-xs font-normal">ريال</span>
                        </p>
                      </div>
                    </div>
                  ))}
